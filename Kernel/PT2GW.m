...
StyleBox[\"PT2GW`\",\nFontWeight->\"Bold\"]\)"];*)
PT2GWPrint@Panel@Row[{Image[Import["PT2GW/icon.png"],ImageSize->Tiny],TableForm[Normal@KeyTake[PacletFind["PT2GW"][[1]]["PacletInfo"],{"Name","Version","Description","WolframVersion","Creator"}]/.Rule[x_,y_]:>{x<>":",y}]},Spacer[20]];
...
Options[ActionRefineInflection]={"NHits"->4,"MaxIterations"->\[Infinity]};
...
pwLaurentFun[data_,Tc_,opt:OptionsPattern[]]:=Module[{optInt,optFit,\[ScriptCapitalD],STint,orders,coeffs,laur,cons,dataFit,laurFit},
	...
	dataFit=data[[-3;;]];
	laurFit=Check[
		NonlinearModelFit[dataFit,{laur[T]/."\!\(\*SubscriptBox[\(T\), \(c\)]\)"->Tc,Subscript[c, Min[orders]]>0}~Join~cons,coeffs,T,optFit],
...
Options[ActionFit]={
	...
	"StopAtFailure"->False,
	"RefineInflection"->{True,"NHits"->4,"MaxIterations"->12},
	"Refine"->False,
