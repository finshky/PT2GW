...
DPFile="data/DarkAbelianHiggs_DR_Expressions.m";
...
SetDirectory[NotebookDirectory[]]; 
StoreDRExpressions["SubRules"->subRules,"PrintDR"->False,"US"->False(*,"File"->"DarkAbelianHiggs_DR_Expressions.m"*)] 
...
(*SetDirectory[NotebookDirectory[]];
LoadDRExpressions["DarkAbelianHiggs_DR_Expressions.m"]; (* load expressions to the DRExpressions symbol *)*)
...
(* ::Input::Initialization::Plain:: *)
(* load a dark photon transition *)
tr=DPModel[3,"Transition"]
AutoComplete[tr];
...
(* ::Input::Initialization::Plain:: *)
colData={"GaugeCouplings"->1,"MassFunctions"->{"Bosons"->{
	scalarMasses[#1,#2][[1]]&,
	scalarMasses[#1,#2][[2]]&,
	vectorMasses[#1,#2][[1]]&
}},"T0"->tr["Tn"],"T"->tr["Tp"]};
...
(* save/load Dataset *)
(*SetDirectory[NotebookDirectory[]];
Compress[DS]>>data/DarkAbelianHiggs_DR_Dataset.m*)
(*Uncompress[<<data/DarkAbelianHiggs_DR_Dataset.m]*)
