...
DRStep;
(* effective potential *)
DefineEffectivePotential;
EffectivePotentialN;
VTot;
ComputeEffectivePotential;
(* plot *)
PlotRG;
PlotDR;
PlotHighTRatio;
...
msg["load"]=StringTemplate["\!\(\*
StyleBox[\"Failed to load \",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)\!\(\*
StyleBox[\" \",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)``\!\(\*
StyleBox[\". Please provide\",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)\!\(\*
StyleBox[\" \",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)\!\(\*
StyleBox[\"a\",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)\!\(\*
StyleBox[\" valid DR file.\",\nStripOnInput->False,\nLineColor->RGBColor[1, 0, 0],\nFrontFaceColor->RGBColor[1, 0, 0],\nBackFaceColor->RGBColor[1, 0, 0],\nGraphicsColor->RGBColor[1, 0, 0],\nFontColor->RGBColor[1, 0, 0]]\)"];
...
append3dRule = #->Symbol[ToString[#]<>"3d"]&;
append3dUSRule = #->Symbol[ToString[#]<>"3dUS"]&;
...
Options[SubRulesAppend]={"Inplace"->True};
AutoComplete[SubRulesAppend];
SetAttributes[SubRulesAppend,HoldFirst];
SubRulesAppend[subRules_,OptionsPattern[]]:=Module[{sub3dRules,subUSRules,subRes},
	...
	If[OptionValue["Inplace"],subRules=subRes,subRes]
]
...
computed by DRalgo in the DRTools` context.
StoreDRExpressions[\"SubRules\"\[Rule]subRules,File\[Rule]file] applies substitution rules and saves to file.";
...
StoreDRExpressions[OptionsPattern[]]:=Module[{
	...
	DRExpressions["\[Phi]Vev"]=DRalgo`Private`\[Phi]Vev["ExplicitValues"];
	(* printint (optional) *)
	If[OptionValue["PrintDR"],
		Print[sepString];
...
LoadDRExpressions[file_]:=Module[{},
	(* initialize DRExpressions Association if not defined *)
	If[!MatchQ[DRExpressions,_Association],DRExpressions=<||>];
	(* load *)
	DRExpressions=Check[Get[file],
		Print@msg["load"][file];
		Abort[]];
	AutoComplete[DRExpressions];
	DRExpressions
...
\[Beta]ToEquations::usage="\[Beta]ToEquations[\[Beta]Rules,\[Mu]] turns a list of rules for \[Beta]-functions to a list of RG equations {\[Mu]\!\(\*SubscriptBox[\(\[PartialD]\), \(\[Mu]\)]\)g\[Equal]\[Beta][g]}.";
\[Beta]ToEquations[\[Beta]Rules_,\[Mu]_]:=Module[{vars,\[Beta]Functions},
...
RGSolve[\[Beta]Functions_List,boundaryValues_,{\[Mu]0_,\[Mu]min_,\[Mu]max_},OptionsPattern[]]:=Module[{out,vars,RGequations,boundaryConditions},
	...
	RGequations=If[Head@\[Beta]Functions[[1]]===Equal,
		\[Beta]Functions,
		\[Beta]ToEquations[\[Beta]Functions,\[Mu]]
...
PlotRG[RGSolutions_List,params_,energyRange_List,opt:OptionsPattern[]]:=Module[{allParams,pars,badParams,expr},
	...
	Plot[Through[expr[\[Mu]]]//Evaluate,{\[Mu],Sequence@@energyRange},
		opt,
		PlotLabels->pars,
		AxesLabel->{"\[Mu]"}
		]
...
Options[PlotDR]={"US"->True};
PlotDR[RGSolutions_List,params_,energyRange_List,s_?NumericQ,opt:OptionsPattern[{DRStep,Plot}]]:=Module[{
...
(* check m/\[Mu] ratio *)
PlotHighTRatio::usage="PlotHighTRatio[RGSolutions,masses3dUS,energyRange,scaleFactor] plots the mass-to-energy scale ratio max(mUS/\[Mu]) in the given energy range.
PlotHighTRatio[masses3dUS,energyRange,scaleFactor] assumes the RGsolutions symbol from DRTools is defined.";
PlotHighTRatio[RGSolutions_List,masses3dUS_,energyRange_List,s_?NumericQ,opt:OptionsPattern[]]:=Module[{
	pars,optDR,optPlot,allParams
	},
	optDR=Evaluate@FilterRules[{opt}~Join~Options[PlotDR],Options[PlotDR]];
	optPlot=Evaluate@FilterRules[{opt},Options[Plot]];
	Plot[Sqrt@Max[masses3dUS/.DRStep[\[Mu]/(s \[Pi]),s,RGSolutions/.((l_->r_)->(l->r[\[Mu]])),optDR]]/\[Mu],
		{\[Mu],Sequence@@energyRange},
		Evaluate[Sequence@@optPlot],
		AxesLabel->{"\[Mu]","\!\(\*SubscriptBox[\(m\), \(US\)]\)/\[Mu]"},
		Epilog->{Dashed,Gray,InfiniteLine[{0,1},{1,0}]}
		]
	]
PlotHighTRatio[masses3dUS_,energyRange_,s_?NumericQ,opt:OptionsPattern[]]:=Module[{},
	If[!ValueQ[RGSolutions],Print["Symbol RGSolutions must be defined or provided as first argument!"];Abort[]];
	PlotHighTRatio[RGSolutions,masses3dUS,energyRange,s,opt]
	]
...
DefineEffectivePotential::usage="DefineEffectivePotential[] constructs the analytic effective potential VTot[\[Phi],order] and defines the \"VTotLO\", \"VTotNLO\", \"VTotNNLO\" entries in DRExpressions.";
DefineEffectivePotential[]:=Module[{vev},
	...
	vev=DRExpressions["\[Phi]Vev"]//First; (* extract vev symbol *)
	(*VTot*)DRExpressions["VTot"]=Function[{\[Phi],order},Switch[order,
		"LO",DRExpressions["VTotLO"]/.vev->\[Phi],
		"NLO",DRExpressions["VTotNLO"]/.vev->\[Phi],
		"NNLO",DRExpressions["VTotNNLO"]/.vev->\[Phi]]//Evaluate];
	AutoComplete[DRExpressions];
	]


EffectivePotentialN::usage="EffectivePotentialN[\[Phi],T,RGSolutions,\[Mu]4DPrefactor] gives the (numerical) effective potential extracted from DRalgo.";
Options[EffectivePotentialN]={"\[Mu]4DPrefactor"->1.,"OrderDR"->"NLO","OrderVeff"->"NNLO","US"->True,"RescaleTo4D"->True,"NumericRules"->{}};
AutoComplete[EffectivePotentialN,{EffectivePotentialN,DRStep}];
EffectivePotentialN[\[CurlyPhi]_,T_,RGsol_,\[Mu]4DPrefactor_,opt:OptionsPattern[{EffectivePotentialN,DRStep}]]:=Module[
	{\[Mu]4d,p4d,p3d,optDR,
		oDR=OptionValue["OrderDR"],oVeff=OptionValue["OrderVeff"],numRules=OptionValue["NumericRules"]
	},
	optDR=Evaluate@FilterRules[{opt}~Join~Options[EffectivePotentialN]~Join~Options[DRStep],Options[DRStep]];
	(*\[Mu]4DPrefactor = OptionValue["\[Mu]4DPrefactor"];*)
...
ComputeEffectivePotential::usage="ComputeEffectivePotential[benchmarkParameters,{\[Mu],\[Mu]min,\[Mu]max}] returns the effective potential V[\[Phi],T].
ComputeEffectivePotential[benchmarkParameters,{\[Mu],\[Mu]min,\[Mu]max},\"SubRules\"\[Rule]subRules] passes substitution rules to StoreDRExpressions. 
ComputeEffectivePotential[benchmarkParameters,{\[Mu],\[Mu]min,\[Mu]max},\"LoadDRFrom\"\[Rule]file] loads DR expressions from file.";
Options[ComputeEffectivePotential]={"SubRules"->None,"OrderDR"->"NLO","OrderVeff"->"NNLO","PlotRG"->False,"LoadDRFrom"->None};
functionList={ComputeEffectivePotential,StoreDRExpressions,EffectivePotentialN};
AutoComplete[ComputeEffectivePotential,functionList];
ComputeEffectivePotential[bchmParams_,energyRange_,opt:OptionsPattern[functionList]]:=Module[{
	...
	optVeff=Evaluate@FilterRules[{opt}~Join~Options[EffectivePotentialN],Options[EffectivePotentialN]]//DeleteDuplicatesBy[#,First]&;
	(* store DRalgo quantities *)
	Which[load=!=None,LoadDRExpressions[load],
		subRules=!=None,StoreDRExpressions["SubRules"->subRules,File->OptionValue[File],optStore],
		True,Print@msg["computeVeff"]
		];
	(* solve RG equations *)
	RGSolutions=RGSolve[DRExpressions["BetaFunctions"],bchmParams,energyRange,optRG];
	If[OptionValue["PlotRG"],Echo[PlotRG[All,Rest@energyRange,ScalingFunctions->"SignedLog",ImageSize->Medium],"RG solutions \[Rule]"]];
	(* effective potential *)
	DefineEffectivePotential[];
	Function[{\[Phi],T},Check[EffectivePotentialN[\[Phi],T,RGSolutions,1,optVeff],
		Limit[EffectivePotentialN[\[CurlyPhi],T,RGSolutions,1,optVeff],\[CurlyPhi]->\[Phi]]
		]//Quiet]
	]
...
Print["Imported \!\(\*StyleBox[\"DRTools`\",\nFontWeight->\"Bold\"]\) \[Checkmark]"];
